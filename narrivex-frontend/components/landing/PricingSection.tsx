  ...
  return (
    <section id="pricing" className="mb-24 scroll-mt-20">
      <div className="mb-8 sm:mb-12 text-center">
        ...
        <div className="mt-6 sm:mt-8 inline-flex items-center gap-1 sm:gap-1.5 rounded-full border border-slate-200 bg-white/80 p-1 shadow-xs dark:border-slate-800 dark:bg-slate-900 max-w-full overflow-x-auto no-scrollbar">
          <button
            type="button"
            onClick={() => setIsAnnual(false)}
            className={`rounded-full px-3 sm:px-4 py-1.5 font-mono text-xs font-semibold transition shrink-0 ${!isAnnual
          ...
          <button
            ...
            className={`inline-flex items-center gap-1 sm:gap-1.5 rounded-full px-3 sm:px-4 py-1.5 font-mono text-xs font-semibold transition shrink-0 ${isAnnual
                ? 'bg-sea text-white shadow-xs font-bold'
                : 'text-slate-600 hover:text-slate-900 dark:text-slate-400 dark:hover:text-white'
              }`}
          >
            <span>Annual Billing</span>
            <span className="rounded-full bg-emerald-400/20 px-1.5 sm:px-2 py-0.2 text-[9px] sm:text-[10px] font-bold text-emerald-600 dark:text-emerald-300">
              SAVE 20%
            </span>
          </button>
        </div>
      ...
      <div className="grid gap-6 lg:grid-cols-3 items-stretch">
        {plans.map((plan) => {
          ...
          return (
            <div
              key={plan.id}
              className={`relative flex flex-col justify-between rounded-2xl p-5 sm:p-7 transition-all duration-200 ${isHighlighted
            ...
            >
              {plan.badge && (
                <div className="absolute -top-3 left-1/2 -translate-x-1/2 rounded-full bg-sea px-3 py-0.5 font-mono text-[9px] sm:text-[10px] font-bold tracking-wider text-white shadow-sm whitespace-nowrap">
                  {plan.badge}
                </div>
              )}
              ...
              <div>
                ...
                <p className="mt-2 text-xs text-slate-600 dark:text-slate-400 min-h-[32px] leading-relaxed">
                  {plan.description}
                </p>
                ...
                <div className="mt-5 border-y border-slate-100 py-4 dark:border-slate-800">
                  ...
                  {isAnnual && typeof plan.monthlyPrice === 'number' && (
                    <span className="text-[10px] sm:text-[11px] font-mono font-semibold text-emerald-600 dark:text-emerald-400 block mt-1">
      ...
      <div className="mt-8 sm:mt-10 rounded-2xl border border-amber-200/80 bg-amber-50/60 p-4 sm:p-5 text-xs text-amber-900 dark:border-amber-900/40 dark:bg-amber-950/20 dark:text-amber-300">
        ...
        <div className="mt-2.5 flex flex-wrap gap-x-4 gap-y-2 font-mono text-[11px] font-semibold text-sea dark:text-emerald-400">
